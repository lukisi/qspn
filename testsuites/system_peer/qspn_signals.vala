...
namespace SystemPeer
{
    ...
    void per_identity_qspn_gnode_splitted(IdentityData id, IQspnArc a, HCoord d, IQspnFingerprint fp)
    {
        ...
        PseudoArc arc = ia.arc;
        int arc_num = -1;
        for (int i = 0; i < arc_list.size; i++)
        {
            PseudoArc _arc = arc_list[i];
            if (_arc == arc)
            {
                arc_num = i;
                break;
        ...
        for (int i = 0; i < 10; i++)
        {
            NodeID _peer_nodeid = fake_random_nodeid(arc.peer_pid, i);
        ...
        assert(peer_id >= 0);
        assert(fp is Fingerprint);
        int64 old_fp = ((Fingerprint)fp).id;
        print(@"Qspn: Identity #$(id.local_identity_index): [$(printabletime())] gnode splitted.\n");
        print(@"      Through identity-arc $(arc_num)+$(peer_id) tell peer_nodeid $(peer_nodeid.id)\n");
        print(@"      that g-node ($(d.lvl), $(d.pos)) has splitted and fp=$(old_fp) must re-enter.\n");
        tester_events.add(@"Qspn:$(id.local_identity_index):Signal:gnode_splitted($(arc_num)+$(peer_id),$(peer_nodeid.id),$(d.lvl),$(old_fp))");
    }
}